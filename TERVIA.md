...
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ssh/`         | The product. `ssh_open/close/attach/list_sessions`, `ssh_shell_open/write/resize/close` (one shell channel per terminal tab), `ssh_confirm_host_key`, `ssh_agent_keys`, `ssh_forward_open/close`, `ssh_remote_forward_open/close`, `ssh_socks_open`, `ssh_git_status`, `ssh_git`, `ssh_sftp_*` (see below), `ssh_key_inspect`, `ssh_key_generate`, `ssh_key_classify`. |
...
| `clipboard.rs` | `clipboard_read_text`: paste reads run host-side via arboard because WebKitGTK rejects `navigator.clipboard.readText()` under Tauri's default webview flags. Writes run host-side too, for the RDP CLIPRDR bridge, over one process-global handle - an X11 selection dies with the handle that owns it. Ordinary app writes stay on the webview API.                   |
...
- `tunnel.ts` owns one refcounted SSH session per saved host, shared by headless
  forwards, RDP-over-jump and every terminal tab (`openShellForConnection`, one
  shell channel each); the last release closes it. It refuses a connection with
  no pinned server key unless the caller passes `promptForHostKey`, because a
  first connect needs a human to answer the dialog.
