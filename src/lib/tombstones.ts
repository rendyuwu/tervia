 ...
 * and a repeated id inside `added` itself keeps the LAST one. The landing carries
 * the remote's `deletedAt`, which is the answer both devices have to agree on;
 * keeping a second copy beside it would leave the list saying one delete happened
 * twice, at two times, and the earlier one would expire first.
 *
 * A LANDING THAT CHANGES NOTHING ALSO ANSWERS `null`. The same delete arriving
 * twice is the ordinary case, not an exotic one - two devices that already agree
 * exchange nothing else - and writing the key for it would rewrite the file on a
 * pull that landed no new information, which is the clobber surface the `null`
...
export function landedTombstones(
  ...
  const deduped = added.filter((t, i) => !added.slice(i + 1).some((o) => o.id === t.id));
  ...
  const next = withTombstone(withoutCleared ?? base, deduped, now);
  return sameTombstones(current, next) ? null : next;
}

/** Whether two lists say the same thing, regardless of order - the question
 *  {@link landedTombstones} has to answer to leave an unchanged key out of a
 *  commit. */
function sameTombstones(a: Tombstone[], b: Tombstone[]): boolean {
  if (a.length !== b.length) return false;
  // `\0` AS AN ESCAPE, not a literal NUL typed into the source. The separator
  // has to be something no `kind` or `id` can contain, and the byte is right -
  // but a raw one in the file makes `git` call this source binary and refuse to
  // show a diff of it at all, so every later change here lands unreviewed.
  const key = (t: Tombstone): string => `${t.kind}\0${t.id}\0${t.deletedAt}`;
 ...
 * belongs at. PRESENT ONLY WHEN A BODY SHOULD ACTUALLY BE WRITTEN: the carry
 * toggle and the pull's own "the body changed" answer are both settled before a
 * landing is built, so a store seeing this field writes it without asking
 * anything further. Absent therefore means NO INFORMATION and never "delete the
 * body" - a device that does not carry bodies wins merges with the field unset,
 * against local bodies that are perfectly good.
 */
 ...
 *
 * FIVE CONDITIONS, and they are the whole set: a landing that names no id, an id
 * that disagrees with the record carrying it, a `record` that is not an object, a
 * tombstone with no usable `deletedAt`, and a tombstone of a kind this argument
 * does not own. A sixth is a deliberate act, not a tidy-up - what is deliberately
 * NOT here is every REFERENCE guard the ordinary mutators run. A rule arriving
 * before the host it names is normal: the landing set is a snapshot of another
 * device's consistent inventory, and the order within one pull is an artifact of
 * a listing.
 *
 * THE ID IS CHECKED AGAINST ITS TYPE, not just against the record, and that one
 * is not symmetry for its own sake. {@link isTombstone} requires a `string` id,
 * so a tombstone landing with a number for an id would be written to the file and
 * then filtered out of every read of it forever - the delete lost on this device,
 * silently, while the caller records the object as applied and never lands it
 * again. A record landing with no id is the same shape one step over: it writes a
 ...
 * what it is. That is also what the last condition checks: a tombstone routed
 * into the wrong array would otherwise delete whatever local record happened to
 * share its id, in a store that never held the record it names. That refusal is
 * the one that reports the TOMBSTONE's kind rather than this argument's, because
 * a refusal names the remote object the caller must not record as applied, and
 * that object's name comes from the kind it was published under.
 *
...
export function landingRefusal<T extends { id: string }>(
  ...
  if (landing.deleted) {
    ...
    if (typeof grave.id !== "string" || grave.id.length === 0) {
      return { kind, id: String(grave.id), reason: "the tombstone names no id" };
    }
    if (grave.kind !== kind) {
      return {
        kind: grave.kind,
  ...
  if (typeof landing.id !== "string" || landing.id.length === 0) {
    return { kind, id: String(landing.id), reason: "the landing names no id" };
  }
