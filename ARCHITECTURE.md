...
| `clipboard.rs`                                      | Clipboard **reads and writes** in the host process (`arboard`): reads because `navigator.clipboard.readText()` cannot work under wry's WebKitGTK defaults, writes because the RDP CLIPRDR bridge needs them and an X11 selection has to stay owned by a live handle. One process-global handle for the app's lifetime. Ordinary app writes stay on the webview API. |
...
connection it accepts back - `SshSession::server_channel_open_forwarded_tcpip`
is what routes that acceptance. `ssh_socks_open(id, local_port)` is `ssh -D`: a
